...
No hosted workflow currently runs these checks: the repository-checks workflow was removed from `main` on 2026-09-13, so `python tools/harness.py check` is run locally before pushing. If a hosted workflow is reintroduced, keep it to the same repository checks with read-only permissions and pinned actions, performing no app builds, voice calls, physical tests, deployment or certification, and maintain its action/runtime versions through the normal review process.
