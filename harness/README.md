...
This repository has no hosted CI by decision: the owner's prior GitHub Actions workflow ran only these same repository checks and was removed on 2026-09-13, matching the app repository's own no-hosted-CI direction ([DEC-008](../spec/decisions/DEC-008-swift-6-language-mode-and-executable-quality-ratchets.md)). Run `check` and `coverage` locally before every push; they perform no app builds, voice calls, physical tests, deployment or certification.
