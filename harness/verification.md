...
| Static/build | Clean reproducible build, compiler/type/lint checks, dependency/security review appropriate to selected stack. For the Trio app this is the gate set of [DEC-008](../spec/decisions/DEC-008-swift-6-language-mode-and-executable-quality-ratchets.md): the app repository's BuildChecks, its quality gate (SwiftFormat, SwiftLint, ratchet comparisons), and an XcodeBuildMCP lane on the exact commit | Exact commit/toolchain; build artifact identity; no ignored failing checks; every program baseline at or moving toward its target |
