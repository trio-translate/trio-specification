...
| Static/build | Clean reproducible build, compiler/type/lint checks, dependency/security review appropriate to selected stack. For the Trio app this is the gate set of [DEC-008](../spec/decisions/DEC-008-swift-6-language-mode-and-executable-quality-ratchets.md): the app repository's BuildChecks, the lint commands and ratchet comparisons its ledger names, and an XcodeBuildMCP lane (or a registered runner plan) on the exact commit | Exact commit/toolchain; build artifact identity; no ignored failing checks; every program baseline at or moving toward its target |
